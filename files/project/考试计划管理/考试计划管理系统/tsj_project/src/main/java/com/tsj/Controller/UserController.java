...
public class UserController {
    ...
    @PostMapping("/forceLogoutNonAdmins")
    public ResponseEntity<Map<String, Object>> forceLogoutNonAdmins(){
        Map<String, Object> response = new HashMap<>();
        
        try {
            // 获取所有在线的非管理员用户
            List<User> onlineUsers = userService.getOnlineNonAdminUsers();
            ...
            for (User user : onlineUsers) {
                ...
                String displayName = user.getRealName() != null && !user.getRealName().isEmpty() 
                    ? user.getRealName() : user.getUsername();
                addUserActivity(displayName + " 因维护模式被强制退出登录");
            }
            
            response.put("success", true);
            response.put("message", "已强制非管理员用户退出登录");
            response.put("loggedOutCount", loggedOutCount);
            
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "强制退出用户失败：" + e.getMessage());
            response.put("loggedOutCount", 0);
            return ResponseEntity.ok(response);
        }
    }
