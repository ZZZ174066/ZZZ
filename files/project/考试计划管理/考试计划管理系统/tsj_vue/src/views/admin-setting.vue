    ...
    <div class="settings-left">
      <div class="settings-card">
        ...
        <div class="settings-content">
          ...
          <div class="setting-item">
            <label>维护模式:</label>
            <el-switch v-model="systemSettings.maintenanceMode" @change="handleMaintenanceModeChange"></el-switch>
          </div>
...
export default {
  ...
  methods: {
    ...
    async handleMaintenanceModeChange(value) {
      const action = value ? '开启' : '关闭'
      const message = value 
        ? '开启维护模式后，除管理员外的所有用户将被强制退出登录，确定要开启维护模式吗？'
        : '确定要关闭维护模式吗？'
      
      try {
        await ElMessageBox.confirm(message, `${action}维护模式`, {
        ...
        // 用户取消，恢复原来的状态
        this.systemSettings.maintenanceMode = !value
      }
    },
    async saveSettings(maintenanceMode = this.systemSettings.maintenanceMode) {
      try {
        ...
        if (res.data.success) {
          ...
          // 通知父组件维护模式状态已改变
          this.$emit('maintenance-mode-changed', maintenanceMode)
          
          // 如果开启维护模式，强制其他用户退出登录
          if (maintenanceMode) {
            await this.forceLogoutNonAdmins()
          }
        } else {
          ElMessage.error('设置保存失败')
          // 保存失败时恢复原状态
          this.systemSettings.maintenanceMode = !maintenanceMode
        }
      } catch (error) {
    ...
    async forceLogoutNonAdmins() {
      try {
        const res = await axios.post('http://localhost:8080/api/user/forceLogoutNonAdmins')
        if (res.data.success) {
          ElMessage.success(`已强制 ${res.data.loggedOutCount} 名用户退出登录`)
        }
      } catch (error) {
        console.error('强制退出用户失败:', error)
        ElMessage.warning('维护模式已开启，但强制退出用户时出现错误')
      }
    },
